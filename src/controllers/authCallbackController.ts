...
import { CONNECTION_URL } from '../services/auth/config'
import { oAuth2Client } from '../services/auth'

const body = (access_token: string | null, refresh_token: string | null) =>
...
export const showToken = async (req: Request, res: Response) => {
	...
	try {
		...
		res.send(body(tokens.access_token ?? null, tokens.refresh_token ?? null))
	} catch (error) {
